...
test('maps every exact Playwright fixture variable without values in logs or files', () => {
  ...
  const expected = ['FLOW4_ADMIN_EMAIL', 'FLOW4_ADMIN_PASSWORD', 'FLOW4_SECOND_ADMIN_EMAIL', 'FLOW4_SECOND_ADMIN_PASSWORD',
    ...
    'FLOW4_CROSS_TENANT_PROOF_URL', 'FLOW4_RETRY_OPERATION_ID', 'FLOW4_ACTIVE_PAYMENT_ID', 'FLOW4_ACTIVE_OPERATION_ID', 'FLOW4_HAPPY_OPERATION_ID',
    'FLOW4_HAPPY_PAYMENT_ID', 'FLOW4_HAPPY_TOKEN', 'FLOW4_PROOF_FILE', 'FLOW4_QUEUE_SEARCH'];
  assert.deepEqual(expected.filter(name => !env[name]), []);
  assert.equal(env.FLOW4_ACTIVE_OPERATION_ID, 'ACTIVE_INVITATION_READY-operation');
  ...
  assert.equal(env.FLOW4_QUEUE_SEARCH, 'INV-F4');
  assert.equal(env.FLOW4_ACTION_REQUIRED_TOKEN, 'REQUEST_INFORMATION_ELIGIBLE-token');
  assert.equal(env.FLOW4_INVALID_EVIDENCE_TOKEN, 'ACTION_REQUIRED-token');
});
